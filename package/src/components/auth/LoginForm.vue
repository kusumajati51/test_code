    ...
    <div>
        <v-row class="mb-3">
            <v-col cols="12">
                ...
                <v-text-field variant="outlined" v-model="login.email" class="pwdInput" hide-details color="primary"></v-text-field>
            </v-col>
            <v-col cols="12">
                <v-label class="font-weight-medium mb-1">Password</v-label>
                <v-text-field variant="outlined" class="border-borderColor" v-model="login.password" type="password" hide-details color="primary"></v-text-field>
            </v-col>
            ...
            <v-col cols="12">
                <v-btn size="large" rounded="pill" color="primary" 
                class="rounded-pill" block type="submit" @click="$emit('login', login)" flat>Sign In</v-btn>
            </v-col>
        </v-row>
    </div>
</template>
